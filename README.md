...
- Generate multirendition **LHLS** with 3 advanced chunks from a test **live** stream and broadcast that stream as a chunked transfer (requires [ffmpeg](https://ffmpeg.org/) and `go-chunked-streaming-server`).
1. First start the `go-chunked-streaming-server`
```
./go-chunked-streaming-server
```
2. Start the multirendition test source:
```
cd scripts
./multi-rendition.sh
```
3. Play the resulting stream with a player that supports LHLS, or you can also play it with any HLS player such Safari, playback URL: `http://localhost:9094/results/playlist.m3u8`
In both cases you will see a latency reduction. In case of an LHLS player you will probably see <1s latency, and in regular HLS players probably you will see a latency similar to target duration.
