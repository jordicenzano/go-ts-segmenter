...
if [ $# -lt 1 ]; then
	echo "Use ./transmuxed-file-to-http.sh FILEin URLhost [Loop]\n"
    echo "FILEin: Input file"
    echo "URLbase: Destination base URL for media chunks (default \"localhost:9094\""
...
# Starts segmenter 
../bin/go-ts-segmenter -inputType 2 -targetDur 4 -manifestDestinationType 0 -mediaDestinationType 3 -host $DST_HOST -chunksBaseFilename source_ &
PID_SRC=$!
